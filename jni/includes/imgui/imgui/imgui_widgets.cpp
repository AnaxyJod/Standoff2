...
void ImGui::DebugNodeInputTextState(ImGuiInputTextState* state)
{
    ...
    if (BeginChild("undopoints", ImVec2(0.0f, GetTextLineHeight() * 15), ImGuiChildFlags_Border)) 
    {
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    const ImU32 col_black = IM_COL32(15, 191, 97,style_alpha8);
...
{
    ImVec4 ThemeColor = ImVec4(198 / 255.f, 120 / 255.f, 87 / 255.f, 0.15f);
    ...
    static std::unordered_map<ImGuiID, float> select_animation;
    float& hover_anim = hover_animation[id];
    float& select_anim = select_animation[id];

    
    const float ANIM_SPEED = 0.15f;
    if (hovered)
        hover_anim = ImMin(hover_anim + g.IO.DeltaTime / ANIM_SPEED, 1.0f);
    else
        hover_anim = ImMax(hover_anim - g.IO.DeltaTime / ANIM_SPEED, 0.0f);

    if (selected)
        select_anim = ImMin(select_anim + g.IO.DeltaTime / ANIM_SPEED, 1.0f);
    else
        select_anim = ImMax(select_anim - g.IO.DeltaTime / ANIM_SPEED, 0.0f);
    ...
    const ImU32 bg_color = GetColorU32(ImGuiCol_WindowBg);
    const ImU32 hover_color = ImColor(25, 25, 26, 230);
    const ImU32 active_color = ImColor(25, 25, 26, 230);
    const ImU32 select_color = GetColorU32(ImVec4(
        ThemeColor.x * 0.7f, 
        ThemeColor.y * 0.7f, 
        ThemeColor.z * 0.7f, 
        ThemeColor.w * 0.5f * select_anim));
    ...
    if (hover_anim > 0.0f)
    {
       ImVec4 col_hover = ImColor(25, 25, 26, 230);
       ImVec4 col_bg_hover = ImColor(25, 25, 26, 230);
       ImVec4 anim_hover_col = ImLerp(col_hover, col_bg_hover, hover_anim);
       ImU32 anim_hover_color = ImColor(25, 25, 26, 230);
        
        if (held)
            window->DrawList->AddRectFilled(bb.Min, bb.Max, active_color, rounding);
        else
            window->DrawList->AddRectFilled(bb.Min, bb.Max, anim_hover_color, rounding);
    }

    
    if (select_anim > 0.0f)
    {
        window->DrawList->AddRect(bb.Min, bb.Max, select_color, rounding, 0, 1.0f * select_anim);
    }
    ...
    const ImVec2 text_padding(style.FramePadding.x + 5, style.FramePadding.y + 5.f);
    RenderTextClipped(bb.Min + text_padding, bb.Max - text_padding, label, NULL, &label_size, style.SelectableTextAlign, &bb);
